    ...
    def block_code(self, code, info=None):
        ...
        # 使用 shift 运行代码
            ...
            if len(infos) > 2:
                ...
                return f"<p><a class='view-message-success view-text-light' target='_blank' rel='nofollow' href='{SHIFT_URL}#lang={infos[0]}&input={b64input}&code={b64code}'>运行示例</a></p><div class='view-monofont'>{highlight(code, lexer, formatter)}</div>"
            else:
                return f"<p><a class='view-message-success view-text-light' target='_blank' rel='nofollow' href='{SHIFT_URL}#lang={infos[0]}&code={b64code}'>运行示例</a></p><div class='view-monofont'>{highlight(code, lexer, formatter)}</div>"
        else:
            try:
                lexer = get_lexer_by_name(infos[0], stripall=True)
            except pygments.util.ClassNotFound:
